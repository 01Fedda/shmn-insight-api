...
(function() {
  ...
  emailPlugin.init = function(config) {
    ...
    emailPlugin.confirmUrl = (
      process.env.INSIGHT_EMAIL_CONFIRM_HOST || config.confirmUrl || 'https://shmnexplorer.info'
    ) + globalConfig.apiPrefix + '/email/validate';

    emailPlugin.redirectUrl = (
      config.redirectUrl || 'https://shmnwallet.com/in/app#!/confirmed'
    );
  };
  ...
  emailPlugin.sendVerificationEmail = function(email, secret, callback) {
    ...
    async.series([
      ...
      var mailOptions = {
        from: 'SHMNWallet@shmnwallet.com',
        to: email,
        subject: '[SHMNWallet] Your wallet backup needs confirmation',
        text: emailBody,
        html: emailBodyHTML
      };
